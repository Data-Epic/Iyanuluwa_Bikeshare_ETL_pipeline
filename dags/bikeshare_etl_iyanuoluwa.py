...
@dag(
    dag_id="bikeshare_etl_iyanuoluwa_v02",
...
def bikesharing_etl():
    ...
    def stream_logs(filepath: str):
        ...
        for trip in stream_trips(df):
            ...
            if duration_mins > 45:
                logger.warning(f"Ride above expected threshold detected! Duration: {duration_mins:.2f} minutes, Ride_ID: {trip['ride_id']}")
            if trip['member_casual'] == 'casual' and trip_start_time == 0:
                logger.warning(f'Midnight ride by a casual rider detected! Ride_id: {trip['ride_id']}, station: {trip['start_station_name']}, end_station: {trip['end_station_name']}')
    ...
    def clean_and_load_parquet(filepath: str):
        ...
        output_partition = f'{LOCAL_PATH}/{CURRENT_TIME}_clean_parquet_partitions.parquet'
